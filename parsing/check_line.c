...
/*   Updated: 2026/01/29 18:19:16 by abdo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	resum_check_line(char *line, char *line_no_spaces, t_data *dt)
{
	if ((ft_strncmp(line_no_spaces, "F", 1) == 0
			|| ft_strncmp(line_no_spaces, "C", 1) == 0)
		&& dt->map_line_start == 0)
	{
	...
	if (dt->nb_no != 1 || dt->nb_so != 1 || dt->nb_we != 1
		|| dt->nb_ea != 1 || dt->nb_f != 1 || dt->nb_c != 1)
...
void	ft_check_line(char *line, t_data *dt)
{
	...
	if ((ft_strncmp(line_no_spaces, "NO", 2) == 0
			...
			|| ft_strncmp(line_no_spaces, "EA", 2) == 0)
		&& dt->map_line_start == 0)
	{
	...
	resum_check_line(line, line_no_spaces, dt);
}
